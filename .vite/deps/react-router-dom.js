...
import { __toESM } from "./chunk-AC2VUBZ6.js";
...
function createRouter(init) {
  ...
  async function startRedirectNavigation(state2, redirect3, _temp) {
    ...
    if (
      ABSOLUTE_URL_REGEX.test(redirect3.location) &&
      isBrowser &&
    ...
    let { formMethod, formAction, formEncType, formData } = state2.navigation;
...
function useActionData() {
  ...
  !route
    ? true
      ? invariant(false, "useActionData must be used inside a RouteContext")
      : invariant(false)
    : void 0;
...
function useSubmitImpl(fetcherKey, fetcherRouteId) {
  let { router } = useDataRouterContext2(DataRouterHook2.UseSubmitImpl);
  let { basename } = React2.useContext(NavigationContext);
...
function useFetcher() {
  ...
  let [Form2] = React2.useState(() => {
    !routeId
      ? true
        ? invariant(false, "No routeId available for fetcher.Form()")
        : invariant(false)
      : void 0;
  ...
  let [load] = React2.useState(() => (href) => {
    !router
      ? true
        ? invariant(false, "No router available for fetcher.load()")
        : invariant(false)
      : void 0;
    !routeId
      ? true
